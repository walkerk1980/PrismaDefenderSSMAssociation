import setuptools


with open("README.md") as fp:
    long_description = fp.read()


setuptools.setup(
    name="prisma_defender_ssm_association",
    version="0.0.1",

    description="A sample CDK Python app",
    long_description=long_description,
    long_description_content_type="text/markdown",

    author="author",

    package_dir={"": "prisma_defender_ssm_association"},
    packages=setuptools.find_packages(where="prisma_defender_ssm_association"),

    install_requires=[
        "pytest",
        "pyyaml",
        "boto3",
        "aws-cdk.core",
        "aws-cdk.aws_lambda",
        "aws-cdk.aws_cloudformation",
        "aws-cdk.aws_ssm",
        "aws-cdk.aws_iam",
        "aws-cdk.aws_s3_assets",
        "aws-cdk.aws_secretsmanager",
        "aws-cdk.aws_codecommit",
        "aws-cdk.aws_codepipeline",
        "aws-cdk.aws_codepipeline-actions",
        "aws-cdk.pipelines"
    ],

    python_requires=">=3.6",

    classifiers=[
        "Development Status :: 4 - Beta",

        "Intended Audience :: Developers",

        "License :: OSI Approved :: Apache Software License",

        "Programming Language :: JavaScript",
        "Programming Language :: Python :: 3 :: Only",
        "Programming Language :: Python :: 3.6",
        "Programming Language :: Python :: 3.7",
        "Programming Language :: Python :: 3.8",

        "Topic :: Software Development :: Code Generators",
        "Topic :: Utilities",

        "Typing :: Typed",
    ],
)
